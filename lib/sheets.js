...
async function getData(spreadsheetId, rangeName) {
  try {
...
async function updateData(spreadsheetId, rangeName, values) {
  try {
    const sheets = getSheetService();
...
async function appendData(spreadsheetId, rangeName, values) {
  try {
    const sheets = getSheetService();
...
async function clearData(spreadsheetId, rangeName) {
  try {
    const sheets = getSheetService();
...
async function deleteRowsByKeyword(spreadsheetId, sheetName, keyword) {
  try {
...
async function findAndReplace(spreadsheetId, sheetName, findText, replaceText) {
  try {
    const fullRange = `${sheetName}!A:Z`;
    const allData = await getRawData(spreadsheetId, fullRange);
...
async function generateAndAddData(spreadsheetId, sheetName, headerRange, numRows, userPrompt) {
  try {
...
async function deleteSheet(spreadsheetId, name) {
  try {
...
async function duplicateSheet(spreadsheetId, sourceSheetName, newSheetName) {
  try {
...
async function sortSheet(spreadsheetId, sheetName, columnIndex, order) {
  try {
...
async function formatCells(spreadsheetId, rangeName, formattingRules) {
  try {
...
async function createChart(spreadsheetId, sheetName, sourceRange, chartType, title) {
  try {
    const sheetId = await getSheetIdByName(spreadsheetId, sheetName);
    if (sheetId === null) return `שגיאה: לא נמצא גיליון בשם '${sheetName}'.`;
...
async function translateToApiCall(userPrompt, spreadsheetId) {
  const openai = new OpenAI({
    apiKey: process.env.OPENAI_API_KEY,
  });
